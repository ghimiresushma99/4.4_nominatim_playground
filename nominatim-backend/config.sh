...
if [[ "$PBF_URL" = "" && "$PBF_PATH" = "" ]]  ||  [[ "$PBF_URL" != "" && "$PBF_PATH" != "" ]]; then
    ...
    # echo "docker run -e PBF_URL=https://drive.usercontent.google.com/u/0/uc?id=1WpYAUkU5rYRtcEynv9w3COWsLLwpYa8m&export=download ..."
    echo "docker run -e PBF_PATH=../data/data.osm.pbf ..."
    exit 1
fi
